...
namespace neural {
  ...
  vector<Neuron> Layer::readNeurons(istream &s, int count) {
    ...
    std::vector<double*> Layer::getWeights() {
        std::vector<double*> pesosLayer;
        for(unsigned int i = 0; i < neurons.size(); i++) {
            std::vector<double *> pesosNeuron = neurons[i].getWeights();
            pesosLayer.insert(pesosLayer.end(),pesosNeuron.begin(),pesosNeuron.end());
            /*for(unsigned int j = 0; j < pesosNeuron.size(); j++) {
                pesosLayer.push_back(pesosNeuron[j]);
            }*/
        }
      return pesosLayer;
